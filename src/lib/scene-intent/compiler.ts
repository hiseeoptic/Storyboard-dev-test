...
export function renderSceneIntentDirective(intent?: SceneIntentIR | null): string {
  ...
  const hook = intent.hook_window.enabled
    ? ` HOOK WINDOW (first ${intent.hook_window.duration_seconds}s): ${intent.hook_window.immediate_visual_event}; ${intent.hook_window.immediate_audio_event}; promise: ${intent.hook_window.core_promise}. Payoff: ${intent.hook_window.payoff_link}.`
    : "";
