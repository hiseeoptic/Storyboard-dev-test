...
test("legacy continuity_mode without transition_in still inherits the previous end snapshot", () => {
  const bd = addContextContracts(twoSegFixture());
  delete bd.segments[1]!.transition_in;
  bd.segments[1]!.continuity_mode = "continuous";
  bd.segments[0]!.state_ledger = {
    start: [{ entity_id: "Minh", state: "eyes half-open", position: "room+bed", orientation: "face-up" }],
    changes: [],
    end: [{ entity_id: "Minh", state: "eyes half-open", position: "room+bed", orientation: "face-up" }],
  };
  bd.segments[1]!.state_ledger = {
    start: [{ entity_id: "Minh", state: "sitting attentive", position: "room+bed", orientation: "facing Lan" }],
    changes: [],
    end: [{ entity_id: "Minh", state: "sitting attentive", position: "room+bed", orientation: "facing Lan" }],
  };

  const normalized = normalizeProductionContracts(bd);
  const start = bd.segments[1]!.state_ledger!.start[0]!;
  assert.equal(normalized.continuous_start_entries_inherited, 1);
  assert.equal(start.state, "eyes half-open");
  assert.equal(start.orientation, "face-up");
  assert.ok(bd.segments[1]!.state_ledger!.changes.some((change) =>
    change.entity_id === "Minh" && /visibly transitions/iu.test(change.action)
  ));
  assert.equal(
    validateStoryboardSemantics(bd).findings.some((finding) => finding.code === "STATE-005"),
    false
  );
});

test("continuous phone holder swap becomes an explicit visible hand-off inside the next shot", () => {
  const bd = addContextContracts(twoSegFixture());
  bd.segments[1]!.transition_in = {
    mode: "continuous", from_location_id: "office", to_location_id: "office",
    time_relation: "immediately", preserve: ["all physical state"], reset: [], reason: "same exchange",
  };
  bd.segments[1]!.continuity_mode = "continuous";
  bd.segments[0]!.state_ledger = {
    start: [{ entity_id: "phone", state: "unchanged", position: "in Lan's hand", holder: "Lan" }],
    changes: [],
    end: [{ entity_id: "phone", state: "unchanged", position: "in Lan's hand", holder: "Lan" }],
  };
  bd.segments[1]!.state_ledger = {
    start: [{ entity_id: "phone", state: "unchanged", position: "in Minh's hand", holder: "Minh" }],
    changes: [],
    end: [{ entity_id: "phone", state: "unchanged", position: "in Minh's hand", holder: "Minh" }],
  };

  normalizeProductionContracts(bd);
  const ledger = bd.segments[1]!.state_ledger!;
  assert.equal(ledger.start[0]!.holder, "Lan");
  assert.match(ledger.changes[0]!.action, /Lan visibly releases phone.*Minh's right hand reaches, contacts, grips and receives phone/iu);
  assert.equal(ledger.changes[0]!.from_holder, "Lan");
  assert.equal(ledger.changes[0]!.to_holder, "Minh");
  assert.equal(
    validateStoryboardSemantics(bd).findings.some((finding) => finding.code === "STATE-005"),
    false
  );
...
test("an explicitly elapsed result is promoted from continuous to an intentional time jump", () => {
  const bd = addContextContracts(twoSegFixture());
  bd.segments[0]!.state_ledger = {
    start: [{ entity_id: "noodle box", state: "sealed", position: "nightstand" }],
    changes: [],
    end: [{ entity_id: "noodle box", state: "sealed", position: "nightstand" }],
  };
  bd.segments[1]!.transition_in = {
    mode: "continuous", from_location_id: "office", to_location_id: "office",
    time_relation: "same moment", preserve: ["all physical state"], reset: [], reason: "continues",
  };
  bd.segments[1]!.continuity_mode = "continuous";
  bd.segments[1]!.first_frame_prompt = "The noodle box is now open with less food after eating.";
  bd.segments[1]!.state_ledger = {
    start: [{ entity_id: "noodle box", state: "open with less food", position: "floor between Minh and Lan" }],
    changes: [],
    end: [{ entity_id: "noodle box", state: "open with less food", position: "floor between Minh and Lan" }],
  };

  const normalized = normalizeProductionContracts(bd);
  assert.equal(normalized.elapsed_boundaries_promoted, 1);
  assert.equal(bd.segments[1]!.transition_in?.mode, "time_jump");
  assert.equal(bd.segments[1]!.continuity_mode, "time_jump");
  assert.equal(bd.segments[1]!.state_ledger!.start[0]!.state, "open with less food");
  assert.equal(
    validateStoryboardSemantics(bd).findings.some((finding) => finding.code === "STATE-005"),
    false
  );
