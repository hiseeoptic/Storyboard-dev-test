...
export const CHARACTER_MOTION_LAWS: Partial<Record<CharacterRepresentation, string>> = {
  whiteboard_stick_figure:
    "MOTION GRAMMAR: communicate acting through readable silhouette poses, spacing, limb arcs, small face-mark changes and sparse motion lines; preserve constant stroke weight and never grow realistic anatomy between poses. Every hand stays a tiny black-line endpoint or minimal mitten mark belonging to the same stick figure; never insert a live-action or photoreal human hand, skin, palm, fingernail, knuckle or wrist.",
  hand_drawn_doodle:
...
export const CHARACTER_LAWS: Record<CharacterRepresentation, string[]> = {
  ...
  whiteboard_stick_figure: ["WHITEBOARD-LINE STICK-FIGURE STYLE LOCK: draw every character with the same clean black stroke weight, circular head, minimal face marks, thin limbs, tiny black-line endpoint or mitten-mark hands, and tiny controlled colour accents. Never insert a realistic human hand or any photographic skin anatomy.", "Translate the complete script-derived location—indoor or outdoor, terrain or architecture, spatial anchors and story props—into the same economical black-line illustration language on a clean light canvas. The canvas is a medium, never a literal blank whiteboard or empty studio; no textured skin, live action, 3D volume or painterly drift."],
