    ...
    if (allBgImages.length > 0) {
      ...
      tasks.push((async () => {
        const desc = await analyzeWithVision({
          ...
          images: allBgImages,
          maxTokens: 380,
          prompt: `These are LOCATION reference photos (${bgNames}). They may show ONE place from multiple angles, or SEVERAL distinct rooms (e.g. a living room and a kitchen) — use the names above to tell them apart, and describe EACH distinct location separately under its name. For each location, reconstruct the space precisely (so an image model can rebuild it identically from any camera angle, including the reverse view): (1) room type and overall geometry (shape, approx size, ceiling); (2) FIXED layout — positions of major furniture/objects relative to each other and to the walls (e.g. "sofa against the back wall, low wooden table centred in front of it, TV console on the right wall, balcony door on the far wall"); (3) walls, floor and materials with precise colours; (4) windows/doors and the direction + warmth of the light; (5) small persistent props that must reappear. Keep it under ~200 words total; state only what is visibly confirmed. This is the authoritative setting reference — be concrete about spatial relationships, not just mood.`,
        });
