...
- Each 10s segment is ONE CONTINUOUS TAKE: a SINGLE primary action filmed in one unbroken shot — never a static scene dragging for 10s, but also NEVER several hard cuts jammed into 10s. AI video models CANNOT "cut"; if you order multiple separate shots inside one clip they MORPH and TELEPORT between them (objects warp, hands pass through props, items appear/vanish). So the "beats" are MOTIVATED CAMERA REFRAMINGS of that SAME ongoing action, paced to its emotion and danger — the subject, props and physics stay continuous for the whole 10s.
...
- Camera movement is controlled and minimal but paced to the story: measured for intimacy, responsive for danger, still when stillness has dramatic purpose. Avoid combining a big camera move with big subject motion — that compounding warps the image.
...
- 🎬 CAMERA VARIETY ACROSS CLIPS: still ONE motivated move or hold per clip, but vary it across the video — push-in / lateral follow / supported arc / pull-back reveal / static tension hold / over-the-shoulder favoring the listener. Speed follows story intensity; NEVER repeat the identical camera recipe in more than two consecutive clips.
...
export function buildVeoJson(
  ...
  const clips = breakdown.segments.map((seg, segIndex) => {
    ...
    const performanceCorpus = `${breakdown.world_context?.genre ?? ""} ${seg.title ?? ""} ${seg.motion_prompt ?? ""} ${seg.scene_intent?.audience_effect ?? ""}`;
    ...
    const performanceMode = actionDrama
      ? "urgent_physical_stakes"
      : /\b(?:comedy|comic|funny|humor|awkward)\b|hài|hài hước|trớ trêu|ngượng/iu.test(performanceCorpus)
        ? "comic_reaction_timing"
        : /\b(?:horror|suspense|fear|threat|danger)\b|kinh dị|hồi hộp|đe dọa|nguy hiểm|sợ hãi/iu.test(performanceCorpus)
          ? "contained_tension"
          : /\b(?:romance|intimate|tender|family|emotional)\b|lãng mạn|thân mật|dịu dàng|gia đình|cảm động/iu.test(performanceCorpus)
            ? "intimate_subtext"
    ...
    const performanceIntent = seg.scene_intent?.performance;
    ...
    return {
      ...
      camera: {
        ...
        movement: actionDrama
          ...
          : `${scrub(cameraMovement)}. One motivated move or hold at story-appropriate human speed; no cuts or separate camera clock. Keep movement physically achievable inside the clip; simplify excess motion instead of rushing it. Hold a readable speaker/listener composition during dialogue and reframe only when dramatically motivated.`,
      ...
      scene_action: {
        ...
        performance_director: {
          ...
          objective: scrub(performanceIntent?.character_objective) || "Pursue the scene objective through visible behavior, not a neutral pose",
          obstacle: scrub(performanceIntent?.obstacle) || "React to the other character, object or event currently changing the scene",
          subtext: scrub(performanceIntent?.subtext) || "Let the face and body reveal what the dialogue does not say directly",
          emotional_arc: `${scrub(performanceIntent?.emotion_start) || "readable opening emotion"} -> ${scrub(performanceIntent?.emotion_end) || "visibly changed end emotion"}`,
          intensity: scrub(performanceIntent?.performance_intensity) || (actionDrama ? "urgent and consequential" : "natural and scene-specific"),
          behavior:
            "Every speaking beat has a clear intention; every visible listener tracks the speaker and gives one small changing reaction through eyes, brow, jaw, breath, hands or weight. Use one motivated gesture at a time, let it complete, and carry its emotional consequence into the end state. No frozen smiles, blank staring, generic pointing, constant nodding or unrelated hand waving.",
        },
