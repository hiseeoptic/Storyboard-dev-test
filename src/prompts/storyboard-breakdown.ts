...
- ENVIRONMENT OVERVIEW (MANDATORY, NEVER OMIT): reserve a bordered WIDE 16:9 thumbnail across the full width of the left library directly below the character rows, occupying at least 18% of the whole board height. Label it "ENVIRONMENT OVERVIEW". ${hasSettingRef ? "The attached LOCATION reference photo(s) are authoritative — each may be an INDOOR room or an OUTDOOR scene. They may be angles of one place or several distinct places — reproduce each exactly (same geometry and depth, placement of every landmark, boundaries, materials, colours, key details, light direction) and, when only one angle is given, infer the place in 3D including its reverse/opposite viewpoint so any panel stays consistent with it. Stage every panel inside the matching uploaded place; never invent a different location. If two places are shown, give each its own small overview thumbnail." : "Derive one stable wide overview from the storyboard setting and reuse that same geometry in every panel."}
