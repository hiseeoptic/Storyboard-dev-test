...
(FULLNESS BUDGET — the user's #1 complaint is empty/curt clips. A TALKING segment carries ~12-18 TOTAL spoken words across 1-2 natural turns that fill most of the 10s. UNDER ~8 words in a conversational beat may be expanded into a fuller in-character exchange (a lead-in → the real point → a reply or reaction) or merged with the adjacent exchange — but only while the complete clip stays within 18 words. Over 18 → push a whole overflow turn into the next compatible segment. Distribute evenly. A beat runs near-silent when the story genuinely needs reflection or a wordless visual/ASMR moment; natural silence is always better than crammed speech. MATCH THE SCRIPT without forcing dialogue into every clip.)
(HARD AUDIO FEASIBILITY OVERRIDE — this overrides every fullness suggestion above: a 10s segment carries AT MOST 18 TOTAL spoken words, normally 12-16, with AT MOST 2 turns. A third turn is allowed only when all three turns still total no more than 18 words. No individual line exceeds 18 words. Budget at least ~0.4s per word plus a real ~0.5s speaker-change gap and leave the final ~0.5s clear. NEVER speed up, cram, repeat or overlap speech to fill a clip. During script polishing, rewrite the same intent more concisely or move a WHOLE turn to an adjacent segment; preserve story facts, speaker ownership and order. A meaningful visual reaction or silence is preferable to physically impossible dialogue.)
...
- HARD SAFETY RULES (a video model CANNOT lip-sync two mouths at once — breaking these causes garbled clips):
  ...
  9. BALANCE THE LOAD ACROSS SEGMENTS (mandatory final audit — unbalanced clips are the #1 cause of dropped/garbled lines): before returning, COUNT the spoken words in every clip. Budget = ~0.4s/word + ~0.5s gap per speaker change + breathing room ⇒ a 10s clip carries at most 18 total spoken words. A clip OVER 18 words → move its last WHOLE turn into the next compatible segment (and shift that segment's lighter lines down). A short conversational line may share its clip with one reply only when the combined total remains ≤18 words. Never squeeze a long line into a short window and never create dialogue merely to eliminate intentional silence. The final distribution should feel natural: no clip nearly silent while its neighbour is crammed, and a clip may stay silent when action or reaction deliberately carries the beat.
  10. HARD AUDIO CAP (FINAL AUTHORITY — overrides any earlier 22-word fullness guidance): each 10s clip has no more than 18 TOTAL spoken words and normally no more than 2 turns; use 3 turns only when their combined total remains ≤18 words. Allocate ≥0.4s/word, ≥0.5s between speakers and finish by 9.5s. Never shorten a timing window to force text to fit. During storyboard expansion, preserve approved wording but distribute WHOLE approved turns across adjacent compatible clips; never duplicate, paraphrase, merge speakers or repeat a line in motion_prompt.
