...
export function buildStoryboardSystemPrompt(
  ...
  const legacyPrompt = `You are a world-class short-form video director and marketing strategist. You design storyboards that are turned into REAL videos using AI image-to-video tools (Google Veo 3 / Veo 3.1, Seedance, Kling).
...
Output MUST be valid JSON only — no markdown, no code fences, no text outside the JSON.${activeRuleDigest ? `\n\n${activeRuleDigest}` : ""}`;
  return compileStoryboardSystemPrompt(legacyPrompt, suppressedRuleIds).prompt;
}
