...
- Give each clip one complete, naturally speakable thought at the locked narrator pace, with room for image, breath and reaction. Do not leave an accidentally thin fragment as the whole clip and do not cram several conclusions into the final clip. Speech ends when the thought ends; the remaining clip is intentional silence and visible continuation, never a repeated utterance.
- Before returning, silently read every VO row against its real clip duration at roughly 128 WPM. This is a delivery audit, not a fixed word quota: move a WHOLE sentence or idea to an adjacent compatible segment when it overruns; when an accidental fragment leaves several empty seconds, deepen the causal bridge, concrete detail or emotional meaning instead of stretching, looping or padding it. Compare all rows and remove any unintentional repeated clause or paraphrased duplicate.
...
(NATURAL SPEECH — CREATIVE AUTHORITY: do NOT count words or force every clip toward a numerical dialogue quota. Write complete, speakable thoughts with character-specific rhythm, subtext, interruption, hesitation, humour or silence when the scene earns them. A short line is valid when its reaction carries the beat; a longer line is valid when it can be delivered naturally. Never pad a scene, flatten a strong line merely to make it shorter, or turn dialogue into polished therapy language.)
(PACING AUDIT: read each exchange aloud in your head against the available clip. Leave room for the visible action and the listener's reaction. If speech feels crowded, move a WHOLE thought or turn to the next compatible segment; never accelerate, overlap, repeat or cut a sentence in half. If it feels empty, deepen the action, subtext or response rather than adding filler.)
