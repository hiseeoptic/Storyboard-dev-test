...
DIALOGUE: natural spoken lines appropriate to character, culture, genre and scene intent. For any conversational or character-driven video, dialogue is the ENGINE — write a real, flowing exchange that FILLS each 10s clip (a genuine back-and-forth with wit, subtext, emotion and specific word choice, not one flat functional line). TWO-PERSON RULE (the user's exact complaint): when 2+ people share a scene, EACH 10s clip must show them INTERACTING — a line AND a reply from a DIFFERENT speaker (2-3 alternating turns inside that one clip). Parking one speaker's line in clip 1 and the other's reply in clip 2 — so every clip has just one person talking — is the FORBIDDEN pattern; keep each little exchange TOGETHER inside its clip and write enough conversation that every talking clip holds a real two-way exchange. A silent or single-thin-line clip is allowed ONLY when the intent is deliberately wordless (ASMR, a pure visual reveal); never leave a talking scene nearly silent. When a beat's natural line is short, EXPAND it into a real exchange (a lead-in → the point → a reply/reaction) rather than shipping a bare one-liner; reserve silence for beats that genuinely need reflection, not as a default. Make every line worth hearing — sharp, human, in-character; SHOW don't tell and never lecture or list without an educational reason.
...
SEGMENT 1 [HOOK WINDOW 3-5s + <PRIMARY FUNCTION justified by project/script>]:
  ...
  DIALOGUE:
    <SpeakerA>: "<opening line that sets up this beat>"
    <SpeakerB>: "<a real reply/reaction — pushes back, teases, or reveals; never a repeat>"
    <SpeakerA>: "<the line that turns or buttons the beat, when it still fits ~9s>"
SEGMENT 2 [<PRIMARY FUNCTION>]:
  IN SCENE: ...
  ACTION: ...
  DIALOGUE:
    <SpeakerB>: "<a fresh beat — lead with a hook, a needle or a confession>"
    <SpeakerA>: "<reply carrying subtext or wit, not a flat acknowledgement>"
    <SpeakerB>: "<optional third turn to fill the 10s>"
(TURN DENSITY — MANDATORY, the user's #1 complaint is thin one-line clips: EVERY talking segment carries 2-3 spoken turns (~12-22 total words) that fill most of the 10s — a real back-and-forth, NOT one line per clip. Reaching the requested segment COUNT never means one thin line per segment: if your natural exchange has fewer total lines than ~2× the number of talking segments, you MUST WRITE MORE — deepen the conversation with genuine reactions, follow-ups, needles, small confessions and fresh beats — until every talking clip is full. Two people who just traded four one-liners across four clips is the DEFECT to avoid; make it four clips of 2-3 turns each instead. A lone 3-8 word line alone in a 10s clip is FORBIDDEN unless the ACTION is deliberately wordless.)
...
- ⛔ NO ONE-SPEAKER-PER-CLIP (the user's #1 rejected defect): in a 2+ person conversation, a 10s clip where only ONE person speaks while the other merely listens is FORBIDDEN. Each talking clip MUST carry a two-way exchange — a line AND a reply from a DIFFERENT speaker (2-3 alternating turns in "dialogue_lines"). If the approved script parked one line per segment (A in clip 1, B's reply in clip 2, …), PACK adjacent lines back together so each clip alternates speakers, and where a clip still holds a lone line, ADD that other character's genuine in-character reply on the same beat so the two of them actually talk. A single talking head in a clip is allowed ONLY for a deliberate solo reflection/monologue beat — never as the default across a conversation.
...
- HARD SAFETY RULES (a video model CANNOT lip-sync two mouths at once — breaking these causes garbled clips):
  ...
  9. BALANCE THE LOAD ACROSS SEGMENTS (mandatory final audit — unbalanced clips are the #1 cause of dropped/garbled lines): before returning, COUNT the spoken words in every clip. Budget = ~0.4s/word + ~0.5s gap per speaker change + breathing room ⇒ a 10s clip carries 8-22 total spoken words. A clip OVER 22 words → move its last turn(s) into the next segment (and shift that segment's lighter lines down); a conversational clip UNDER ~10 words → EXPAND it into a fuller in-character exchange (2-3 turns that fill the 10s) or, if its line belongs to an adjacent clip's conversation, merge them into one clip's dialogue_lines — never leave a talking clip with a lone 3-6 word line and dead air. A SINGLE turn longer than ~22 words must be that clip's ONLY line — never squeeze a 24-word line into a 4-second window and never pair it with another turn. The final distribution should feel even: no clip nearly silent while its neighbour is crammed, and a clip stays truly silent only when its beat is deliberately wordless.
...
10. Return valid JSON only in this exact wrapper: {"segments":[...],"character_locks":[...]} — use an empty array for a target kind that is absent; no markdown, critique, explanation, confidence score or extra keys.`;
}
