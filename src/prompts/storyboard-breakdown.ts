...
- ENVIRONMENT OVERVIEW (MANDATORY, NEVER OMIT): reserve a bordered WIDE 16:9 thumbnail across the full width of the left library directly below the character rows, occupying at least 18% of the whole board height. Label it "ENVIRONMENT OVERVIEW". ${hasSettingRef ? "One or more attached LOCATION reference photos show the SAME real place from different angles — combine them to reconstruct one consistent set and reproduce it exactly: same room geometry, furniture placement, doors/windows, materials, colours, key props and light direction. Reuse that identical set in EVERY panel; this uploaded place is the only environment source — never invent a different room." : "Derive one stable wide overview from the storyboard setting and reuse that same geometry in every panel."}
